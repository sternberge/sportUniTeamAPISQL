...
module.exports = {
  ...
  create(req, res, next) {
    ...
    db.pool.getConnection((error, connection) => {
      ...
      var query = connection.query('INSERT INTO DoubleRankingHistory (DoubleTeams_doubleTeamId, rank, rankPoints, differenceRank, differencePoints, type, date) VALUES(?, ?, ?, ?, ?, ?, ?)',
      [DoubleTeams_doubleTeamId, rank, rankPoints,	differenceRank, differencePoints, type, date], (error, results, fields) => {
        if (error){
          connection.release();
          return res.send(JSON.stringify({"status": 500, "error": error, "response": null}));
        }
