...
module.exports = {
  ...
  getMatchsByTournamentSpringFallYearGender(req, res, next) {
    ...
    db.pool.getConnection((error, connection) => {
      ...
      var query = connection.query('SELECT concat(u1.firstName,\' \',u1.lastName) as WinnerName,concat(u2.firstName,\' \',u2.lastName) as LoserName,sm.*,t.name as TournamentName,c1.name as winnerCollege,c2.name as loserCollege,sr1.rank as winnerRank,sr2.rank as loserRank,c1.collegeId as winnerCollege,c2.collegeId as loserColleger FROM SimpleMatches sm INNER JOIN Players p1 on p1.playerId = sm.winner INNER JOIN Players p2 on p2.playerId = sm.loser INNER JOIN Users u1 on u1.userId = p1.Users_userId INNER JOIN Users u2 on u2.userId = p2.Users_userId INNER JOIN Teams t1 on t1.teamId = p1.Teams_teamId INNER JOIN Teams t2 on t2.teamId = p2.Teams_teamId INNER JOIN Colleges c1 on c1.collegeId = t1.Colleges_collegeId INNER JOIN Colleges c2 on c2.collegeId = t2.Colleges_collegeId INNER JOIN Tournaments t on t.tournamentId = sm.Tournaments_tournamentId LEFT join SingleRanking sr1 ON sr1.Players_playerId = p1.playerId LEFT join SingleRanking sr2 ON sr2.Players_playerId = p2.playerId WHERE sm.Tournaments_tournamentId = ? AND sm.springFall = ? AND sm.date >= \'?-09-01\' AND sm.date <= \'?-06-30\' AND t.gender LIKE ? AND sr1.type = \'N\' AND sr2.type = \'N\' ORDER BY sm.date DESC;', [tournamentId,springFall,year,yearPlusOne,gender], (error, results, fields) => {
        if (error){
          connection.release();
          return res.send(JSON.stringify({"status": 500, "error": error, "response": null}));
        }
  ...
  getMatchsByTournamentCollegeSpringFallYearGender(req, res, next) {
    ...
    db.pool.getConnection((error, connection) => {
      ...
      var query = connection.query('SELECT concat(u1.firstName,\' \',u1.lastName) as WinnerName,concat(u2.firstName,\' \',u2.lastName) as LoserName,sm.*,t.name as TournamentName,sm.score,c1.name as winnerCollege,c2.name as loserCollege,sr1.rank as winnerRank,sr2.rank as loserRank,c1.collegeId as winnerCollege,c2.collegeId as loserColleger FROM SimpleMatches sm INNER JOIN Players p1 on p1.playerId = sm.winner INNER JOIN Players p2 on p2.playerId = sm.loser INNER JOIN Users u1 on u1.userId = p1.Users_userId INNER JOIN Users u2 on u2.userId = p2.Users_userId INNER JOIN Teams t1 on t1.teamId = p1.Teams_teamId INNER JOIN Teams t2 on t2.teamId = p2.Teams_teamId INNER JOIN Colleges c1 on c1.collegeId = t1.Colleges_collegeId  INNER JOIN Colleges c2 on c2.collegeId = t2.Colleges_collegeId INNER JOIN Tournaments t on t.tournamentId = sm.Tournaments_tournamentId LEFT join SingleRanking sr1 ON sr1.Players_playerId = p1.playerId LEFT join SingleRanking sr2 ON sr2.Players_playerId = p2.playerId WHERE (t1.Colleges_collegeId = ? OR t2.Colleges_collegeId = ?) AND sm.Tournaments_tournamentId = ? AND sm.springFall = ? AND sm.date >= \'?-09-01\' AND sm.date <= \'?-06-30\'AND t.gender LIKE ? AND sr1.type = \'N\' AND sr2.type = \'N\'  ORDER BY sm.date DESC;', [collegeId,collegeId,tournamentId,springFall,year,yearPlusOne,gender], (error, results, fields) => {
        if (error){
          connection.release();
          return res.send(JSON.stringify({"status": 500, "error": error, "response": null}));
        }
